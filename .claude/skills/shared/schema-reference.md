...
| `addedLoad` | `true` | No | Only on a `bodyweight` exercise with metric `load` (weighted pull-up, dip). `weight` is then the **external load added to body weight** — 0 = bodyweight only; body weight is never included. Absent on every record before 2026-09-16: a legacy bodyweight set with weight 0 does **not** mean "added 0". History key includes the flag, so added-load and bodyweight-only history are separate. No e1RM or tonnage is meaningful; judge progress by heaviest added load at comparable reps. Assisted (negative) load is not supported |
