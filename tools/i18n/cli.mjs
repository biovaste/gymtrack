...
export async function releaseCache(root,check=false) {
  ...
  const contents=await Promise.all(['index.html','styles.css','app.js','workout-model.js','i18n.js','exercises.js','locales/catalog.js','manifest.webmanifest'].map(async f=>canonical(await readFile(path.join(root,f),'utf8').catch(e=>{if(e.code==='ENOENT')return '';throw e;}))));
